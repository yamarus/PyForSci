...
#. Напишите функцию, ищущую сумму всех значений ``E`` от значений множества значенний ``T, Y`` (``T,Y`` - тип ``list``) :math:`E = \left[y - A \exp(-kt)\right]^2`, где ``A``, ``k`` - основные аргументы фугкции, а ``T`` и ``Y`` передаются в виде ``*args``. Придумайте и реализуйте способ проверки вашей функции. 
