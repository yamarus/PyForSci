...
#. Рассчитайте количество символов, и количество уникальных символов в введенной строке, не включая знаки препинания (,.!?;:) и пробелы.

    .. hint:: используйте типы ``list`` и ``set`` (списки и множества), команду ``len()`` и проверку наличия элемента ``s in reference_list``.
