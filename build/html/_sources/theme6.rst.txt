...
#.  Одним из простейших методов для обработки разупорядоченных данных является метод главных компонент (principal component analysis, PCA). Данный метод позволяет выявить скрытые линейные зависимости в данных, что может помочь с разделением пересекающихся спектров, обработкой изображений и многое другое.

    Суть метода заключается в нахождении собственных векторов и собственных чисел N*N матрицы ковариации N отдельных признаков. При этом, собственные вектора задают направления (комбинации исходных признаков) для наиболее полного ортагонального разложения признаков, а соответствующие собственные значения отображают насколько часто в представленных данных встречается такая комбинация.

    Представим, что у вас есть набор точек, отображающий смещение атома по плоскости в результате диффузии. Вы знаете, что движение в такой среде анизотропно, однако не знаете как лучше задать выделенные направления. Проведя анализ главных компоент, вы получите два собственных ветора и соответствующих им собственных числа (задача то двумерная). В этом случае, напралвние собственных векторов будет определять основные оси анизотропии свойств, а собственные значения - коэффициенты диффузии. 
